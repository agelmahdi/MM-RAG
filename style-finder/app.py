...
class StyleFinderApp:
    ...
    def __init__(self, dataset_path, serp_api_key=None):
        ...
        self.llm_service = LlamaVisionService(config.LLAMA_MODEL_ID)
    ...
    def process_image(self, image):
        ...
        user_enconding = self.image_processor.encode_image(image_path, is_url=False)
        if user_enconding['vector'] is None:
            return "Error: unable to process the image. please try anomter image"
        
        # Find the closest match in the dataset
        closest_row , similarity_score = self.image_processor.find_closest_match(user_enconding['vector'], self.data)
        ...
        response = self.llm_service.generate_fashion_response(
            user_enconding['base64'],
